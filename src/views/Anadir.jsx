...
export default function Anadir({ setVistaActual, refrescarVinilos }) {
  ...
  const guardarVinilo = async (e) => {
    ...
    try {
      ...
      // Limpiar y refrescar
      setTitulo(''); setAutor(''); setAño(''); setGenero(''); setValoracion(0); setCanciones(''); setArchivo(null);
      if (e.target) e.target.reset()
      
      refrescarVinilos() // Actualiza la lista en App.jsx
      
      setTimeout(() => {
        setVistaActual('coleccion') // Llevamos a Edu a ver su nuevo disco
      }, 1500)
  ...
  return (
    <div style={estilos.contenedor}>
      <h1 style={estilos.tituloPagina}>Añadir a la colección</h1>
      
      <form onSubmit={guardarVinilo} style={estilos.formulario}>
        
        <div style={estilos.grupo}>
          <label style={estilos.label}>Título del álbum</label>
          <input type="text" value={titulo} onChange={(e) => setTitulo(e.target.value)} required style={estilos.input} placeholder="Ej: The Dark Side of the Moon" />
        </div>

        <div style={estilos.grupo}>
          <label style={estilos.label}>Artista / Grupo</label>
          <input type="text" value={autor} onChange={(e) => setAutor(e.target.value)} required style={estilos.input} placeholder="Ej: Pink Floyd" />
        </div>

        <div style={estilos.filaDoble}>
          <div style={estilos.grupo}>
            <label style={estilos.label}>Año</label>
            <input type="number" value={año} onChange={(e) => setAño(e.target.value)} style={estilos.input} placeholder="Ej: 1973" />
          </div>
          <div style={estilos.grupo}>
            <label style={estilos.label}>Género</label>
            <input type="text" value={genero} onChange={(e) => setGenero(e.target.value)} style={estilos.input} placeholder="Ej: Rock Progresivo" />
          </div>
        </div>
          ...
          <div style={estilos.contenedorEstrellas}>
            {[1, 2, 3, 4, 5].map((num) => (
              <span 
                ...
                style={{
                  ...estilos.estrella,
                  color: num <= valoracion ? tema.acento : tema.borde
                }}
              >
                {num <= valoracion ? '★' : '☆'}
              </span>
        ...
        <div style={estilos.grupo}>
          ...
          <textarea value={canciones} onChange={(e) => setCanciones(e.target.value)} style={estilos.textarea} rows="4" placeholder="1. Speak to Me&#10;2. Breathe" />
        </div>
        ...
        <button type="submit" disabled={cargando} style={estilos.botonGuardar}>
          {cargando ? 'Guardando...' : 'Añadir Vinilo'}
        </button>
...
const estilos = {
  ...
  // Inputs estilo oscuro (parecido a los de Spotify)
